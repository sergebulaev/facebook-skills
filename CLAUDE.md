...
- **Skill names are public surface.** Renaming a skill is a major version bump and
  ...
  table, and every `fb-<name>` cross-reference in sibling SKILL.md files.
...
- Canonical voice rules live at root `references/voice-rules.md`. Skill-local
  ...
  the under-80 sweet spot, comment rules) and start with: `Global voice rules:
  see root SKILL.md Voice rules.`
- Other root-level references shared across skills:
  `references/hook-formulas.md` (10 Facebook formulas) and
...
- `fb-humanizer` has `sub-skills/` for the folded-in post-audit workflow. Don't
...
```bash
python3 -c "from lib import publish, illustrate, refine, PixfaroClient, parse_facebook_url, PubloraClient; print('OK')"
python3 scripts/sync_codex_marketplace.py
